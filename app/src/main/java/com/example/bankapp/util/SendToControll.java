...
public class SendToControll {
    public void SendControll(String order, String type) {
        try {
            ECMessage msg = ECMessage.createECMessage(ECMessage.Type.TXT);
            // msg.setForm("123456");//设置消息发送者
            msg.setMsgTime(System.currentTimeMillis());
            msg.setTo(Constant.OTHERS_NAME);// 设置消息接收者
            msg.setSessionId(Constant.OTHERS_NAME);//接收者ID
            // 设置消息发送类型（发送或者接收）
            ...
            manager.sendMessage(msg, new ECChatManager.OnSendMessageListener() {
                ...
                public void onSendMessageComplete(ECError error, ECMessage message) {
                    ...
                    // 将发送的消息更新到本地数据库并刷新UI
                    Log.e("WWDZ", "发送成功 ： " + message);
                }
            });
        } catch (Exception e) {
            // 处理发送异常
            Log.e("WWDZ", "send message fail , e=" + e.getMessage());
        }
    }

    public void SendLocal(String filePath, String type) {
        ...
        if (file.exists()) {
            try {
                ...
                // 设置附件名
                msgBody.setFileName("localVoice.txt");
                // 设置附件扩展名
                msgBody.setFileExt("txt");
                // 设置附件本地路径
